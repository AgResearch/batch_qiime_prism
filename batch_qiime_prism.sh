...
function run_prism() {
   ...
   if [ -f $OUT_DIR/qiime_uclust/combined_rep_set_otu_table_L10.txt  ]; then
      ...
      tardis --shell-include-file $OUT_DIR/configure_qiime_env.src biom convert -i $OUT_DIR/qiime_uclust/combined_rep_set_otu_table.txt -o $OUT_DIR/qiime_uclust/table.from_biom_w_taxonomy.txt --to-tsv --header-key taxonomy 1\>$OUT_DIR/qiime_uclust/from_biom_w_taxonomy.stdout  2\>$OUT_DIR/qiime_uclust/from_biom_w_taxonomy.stderr
      if [ $? != 0 ]; then
...
function clean() {
   nohup rm -rf $OUT_DIR/tardis_* > $OUT_DIR/clean.log 2>&1 &
}
